...
## Efficiency of multi=TRUE has been greatly improved in ribiosUtils >= 1.0-16
...
system.time(freeProbesetsAllOrtho <- annotateAnyProbeset(ids=myProbesets$ProbeID,
                                                         orthologue=TRUE))
## annotateing orthologues is fast enough from ribiosUtils >= 1.0-16
...
system.time(guessedTypes <- guessChiptype(myProbesets$ProbeID, sample=10))
